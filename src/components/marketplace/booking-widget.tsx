...
export function BookingWidget({
  ...
  // Slot availability and the selected slot are keyed to the exact
  // provider/service/date combination they were fetched for, so loading and
  // stale-selection states are derived instead of set synchronously in an effect.
  const requestKey = `${providerId}|${serviceId}|${date}`;
  const [slotResult, setSlotResult] = useState<{ key: string; slots: string[] } | null>(null);
  const [selection, setSelection] = useState<{ key: string; slot: string } | null>(null);
  const [notes, setNotes] = useState("");

  const loadingSlots = !slotResult || slotResult.key !== requestKey;
  const slots = slotResult && slotResult.key === requestKey ? slotResult.slots : [];
  const selectedSlot = selection && selection.key === requestKey ? selection.slot : null;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const key = `${providerId}|${serviceId}|${date}`;
    fetch(`/api/providers/${providerId}/slots?date=${date}&serviceId=${serviceId}`)
      .then((res) => res.json())
      .then((data) => {
        if (!cancelled) setSlotResult({ key, slots: data.slots ?? [] });
      })
      .catch(() => {
  ...
  async function handleBook() {
    ...
    try {
      ...
      setSlotResult((prev) => (prev ? { key: prev.key, slots: prev.slots.filter((s) => s !== selectedSlot) } : prev));
      setSelection(null);
    } finally {
      setSubmitting(false);
    }
            ...
            <div className="grid max-h-40 grid-cols-3 gap-2 overflow-y-auto pr-1">
              {slots.map((slot) => (
                <button
                  key={slot}
                  type="button"
                  onClick={() => setSelection({ key: requestKey, slot })}
                  className={cn(
